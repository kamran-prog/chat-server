...
async function testServerIntegration() {
  ...
  // Room message — recipient sees it
  const roomMsgReceived = new Promise((resolve) => {
    const handler = (data) => {
      if (!data.system && !data.self) { clientA.off(EVENTS.ROOM_MESSAGE, handler); resolve(data); }
    };
  ...
  assert(roomMsg !== null && roomMsg.content === "Hey room!", "room message delivered to other members");

  // Room message — sender gets self echo (Bug 1 fix)
  const selfRoomMsg = new Promise((resolve) => {
    const handler = (data) => {
      if (data.self) { clientA.off(EVENTS.ROOM_MESSAGE, handler); resolve(data); }
    };
    clientA.on(EVENTS.ROOM_MESSAGE, handler);
  });
  clientA.emit(EVENTS.ROOM_MESSAGE, { room: "lobby", content: "My own message" });
  const selfEcho = await Promise.race([selfRoomMsg, wait(3000).then(() => null)]);
  assert(selfEcho !== null, "room message sender receives self echo");
  assert(selfEcho && selfEcho.self === true, "self echo has self: true flag");
  assert(selfEcho && selfEcho.content === "My own message", "self echo content matches");
  ...
  // Group invite by non-admin should fail gracefully (Bug 2 fix)
  const forbiddenErr = new Promise((resolve) => {
    const handler = (data) => {
      if (data.code === "FORBIDDEN") { clientB.off(EVENTS.ERROR, handler); resolve(data); }
    };
    clientB.on(EVENTS.ERROR, handler);
  });
  clientB.emit(EVENTS.GROUP_INVITE, { groupId, userId: "user-X" });
  const forbid = await Promise.race([forbiddenErr, wait(3000).then(() => null)]);
  assert(forbid !== null && forbid.code === "FORBIDDEN", "non-admin invite returns FORBIDDEN (no crash)");

  // Group kick by non-admin should fail gracefully (Bug 2 fix)
  const kickForbidden = new Promise((resolve) => {
    const handler = (data) => {
      if (data.code === "FORBIDDEN") { clientB.off(EVENTS.ERROR, handler); resolve(data); }
    };
    clientB.on(EVENTS.ERROR, handler);
  });
  clientB.emit(EVENTS.GROUP_KICK, { groupId, userId: "user-C" });
  const kickForbid = await Promise.race([kickForbidden, wait(3000).then(() => null)]);
  assert(kickForbid !== null && kickForbid.code === "FORBIDDEN", "non-admin kick returns FORBIDDEN (no crash)");

  // Group kick (owner kicks user-C) — should still work for admins
